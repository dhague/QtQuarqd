...
static const unsigned char NetworkKey[8] = { 0xB9, 0xA5, 0x21, 0xFB, 0xBD,
        0x72, 0xC3, 0x45 };
...
void writeData(char *str) {
    ...
    for (i = 0; i < num_clients; i++) {
        if (0 == writeDataSingleChannel(str, i))
            i--;
    }
}
...
int act_on_incoming_message(char *key, char *value, int connection) {
    ...
    if ((0 == strcmp(key, "X-set-channel")) || (0 == strcmp(key,
            "X-pair-device"))) {
        ...
        if (0 == strcmp(key, "X-pair-device"))
            channel_type |= CHANNEL_TYPE_PAIR;

        return channel_manager_add_device(&cm, device_number, channel_type,
                channel_number);
        ...
        if (channel_type == CHANNEL_TYPE_POWER) { // special case for quarq
            ...
            ret = (channel_manager_remove_device(&cm, device_number,
                    CHANNEL_TYPE_POWER) | channel_manager_remove_device(&cm,
                    device_number, CHANNEL_TYPE_QUARQ)
                    | channel_manager_remove_device(&cm, device_number,
        ...
        for (c = value; *c; c++)
            if (*c == '=') {
                *c = '\0';
                ...
                if (errno)
                    value = "unknown"; // force value setting to fail
                break;
            }

        if ((0 != strcmp(value, "blanking")) && (0 != strcmp(value, "drop"))
                && (0 != strcmp(value, "scan")) && (0 != strcmp(value, "lost"))) {
    ...
    } else if ((0 == strcmp(key, "X-calibrate")) || (0 == strcmp(key,
            "X-get-slope")) || (0 == strcmp(key, "X-get-offset")) || (0
            == strcmp(key, "X-set-slope")) || (0 == strcmp(key,
            "X-set-test-mode")) || (0 == strcmp(key, "X-unset-test-mode"))) {
        ...
        if ((channel_type != CHANNEL_TYPE_POWER) && (channel_type
                != CHANNEL_TYPE_FAST_QUARQ) && (channel_type
                != CHANNEL_TYPE_FAST_QUARQ_NEW)) {
            SendErrorMessage(connection,"%s requested on a non-power sensor",key);
            return 0;
        }

        ant_channel_t *ac = channel_manager_find_device(&cm, device_number,
                channel_type);

        if (NULL == ac) {
            ac = channel_manager_find_device(&cm, device_number,
                    CHANNEL_TYPE_FAST_QUARQ);
        }
...
int main(void) {
    ...
    // Main server loop - should move to TcpServerThread
    while (!quit) {
        int i;
        struct timeval tv;
        int ret;
